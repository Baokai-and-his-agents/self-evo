...
- 把任务、链接、技能名写入文档。
- agent 定时读取文档并执行一个或多个任务。
- agent 把执行结果、阻塞点、需要确认的动作写回文档。
- 用户再通过修改文档继续指导它。

这让系统不依赖某一次聊天上下文，可以跨轮次、跨 worker、跨日期持续运行。

## 3. 第一性原则

### 3.1 文件是事实源

长期记忆、任务队列、决策记录、技能沉淀都应优先存在于 Git 管理的 Markdown / YAML / JSONL 文件中。
...
除了本地文档协议，项目可以使用 GitHub 作为人和 agent、多 agent 之间的共享状态协调层。

这个设计是可行的，但要明确 GitHub 的定位：
...
- GitHub issue 创建后，可以由 worker 同步到 docs/TASKS.md。
- 本地 INBOX.md 中的新任务，可以由 supervisor 创建 GitHub issue。
- PR 合并后，worker 更新 docs/RUNLOG.md 和 memory/runs。
- 重要长期结论进入 docs/DECISIONS.md 或 memory/project/decisions.md。
...
不建议第一版让一个 Claude Code 会话永远不间断地跑。更稳的是用定时任务唤醒一个短周期 worker。
...
记忆系统采用“文件为主、索引为辅、定期压缩、可人工纠偏”的设计。
...
reviewer 或 supervisor 复核
  ↓
合并到 profile / project / skills / learnings
```
...
### 7.3 LLM Wiki / OpenViking / GBrain 的位置
...
- 第一版是否只支持单 worker，还是直接预留多 worker？
...
- GitHub issue 是唯一任务入口，还是和本地 INBOX.md 双向同步？
- 是否为每个 worker 创建独立 GitHub bot/user，还是用同一个账号加 worker identity comment？
- GitHub Project 字段是否第一版就使用，还是先只用 issue labels？
...
3. 创建 docs/TASKS.md
4. 创建 docs/REVIEW.md
5. 创建 memory/profile/permissions.yaml
6. 创建 scripts/tick.ps1
```
...
Read docs/INBOX.md and docs/TASKS.md.
Claim exactly one open task.
Execute only within the allowed permission level.
Write docs/RUNLOG.md and docs/REVIEW.md before stopping.
```
