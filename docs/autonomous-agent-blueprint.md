...
v0.3 在 v0.2 的基础上，进一步收敛第一版的安全边界和协作方式：
...
v0.3 确认：第一版任务入口以 GitHub Issue 为主。GitHub 是人和 agent、多 agent 之间的共享状态协调层；本地 rules / data 是规则、长期记忆和执行证据层。
...
```text
SessionStart
  加载 rules/AGENT_PROTOCOL.md、检查环境、写 heartbeat。
...
2. rules/PERMISSIONS.yaml 和 rules/RESOURCE_APPROVALS.yaml
3. data/memory/hot/profile/user_preferences.md
4. data/memory/hot/project/goals.md 和 data/memory/hot/project/decisions.md
5. 当前任务
6. 相关 data/memory/hot skill memory
7. 最近 1-3 次 run summary
8. 必要时检索 data/memory/cold 或 index
```
...
v0.3 对探索的态度是：允许适当发散，但必须可复审。Token 和网络资源充裕时，可以扩大阅读和扫描范围；真正的约束不是“少看”，而是“看过什么、为什么看、为什么保留或丢弃”必须记录下来。
...
主动 agent 必须有预算机制。v0.3 中预算不是为了压制探索，而是为了控制安全边界、资源上限和复盘粒度。由于用户允许较充裕的 token 和网络资源，探索预算可以相对宽松，但外部付费资源仍必须遵守资源审批表。
...
```text
1. 创建 rules/AGENT_PROTOCOL.md
2. 创建 rules/PERMISSIONS.yaml
3. 创建 rules/RESOURCE_APPROVALS.yaml
4. 创建 data/tasks/TASKS.md，作为 GitHub Issue 本地镜像
5. 创建 data/tasks/REVIEW.md
6. 创建 data/memory/hot 和 data/memory/cold 基础目录
7. 创建 data/proposals/rule_changes
8. 创建 .github/CODEOWNERS
9. 配置 main 分支保护和 required code owner review
10. 创建 GitHub Issue template
```

然后让本地 Code worker 执行第一条固定指令：

```text
Read rules/AGENT_PROTOCOL.md.
Read rules/PERMISSIONS.yaml and rules/RESOURCE_APPROVALS.yaml.
...
Write an Issue comment, data/runs and data/tasks/REVIEW.md before stopping.
```
